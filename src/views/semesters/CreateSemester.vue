<template>
  <v-container>
    <v-row>
      <v-col class="mt-8">
        <AppBar title="Create Semester"/>

        <v-card class="mt-4">
          <v-card-title>Create Semester</v-card-title>
          <v-card-text>
        ...
        <v-row class="mt-4">
          <v-col class="d-flex justify-end">
            ...
            <v-btn color="primary" :loading="submitting" @click="submit">Create</v-btn>
          </v-col>
        </v-row>
...
export default {
  ...
  methods: {
    async submit() {
      ...
      try {
        await createSemester(this.form)
        this.$router.push({ name: 'semester-management' })
      } catch (error) {
        this.errorDialog = {
          visible: true,
          title: 'Save Failed',
          message: error.response?.data?.message || error.message || 'Failed to create semester',
        }
      } finally {

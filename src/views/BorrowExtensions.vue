<template>
    ...
    <v-container fluid class="pa-6">
            ...
            <v-col cols="12" md="6">
                <v-card elevation="2" class="pa-4">
                    <v-row class="mb-4" align="center">
                        <v-col>
                            <h3 class="mb-0">Extend Borrowing</h3>
                            <div class="text-subtitle-2 text-grey">Search or scan a borrow record to begin.</div>
                        </v-col>
                        <v-col cols="auto">
                    ...
                    <v-row>
                        ...
                        <v-col cols="12" class="mt-4">
                            <v-card elevation="0" class="pa-4">
                                <div class="text-subtitle-1 font-weight-bold mb-2">Selected Borrow</div>
                                <div v-if="selectedBook">
                                    ...
                                    <p class="mb-1" v-if="selectedBook.publisher"><strong>Publisher:</strong> {{ selectedBook.publisher }}</p>
                                    <p class="mb-1" v-if="selectedBook.yearOfProduction"><strong>Year:</strong> {{ selectedBook.yearOfProduction }}</p>
                                    <p class="mb-1" v-if="selectedBook.edition"><strong>Edition:</strong> {{ selectedBook.edition }}</p>
                                    <p class="mb-1" v-if="selectedBook.placeOfPublication"><strong>Place:</strong> {{ selectedBook.placeOfPublication }}</p>
                                    <p class="mb-1" v-if="selectedBook.notes"><strong>Notes:</strong> {{ selectedBook.notes }}</p>
                                </div>
                                <div v-else class="text-grey">No borrow selected.</div>
                                <div class="mt-4">
                                    <v-row>
                                        <v-col cols="12" md="6">
                                            <div v-if="currentBorrow">
                                                        <div v-if="currentBorrow.is_penalized && currentBorrow.status === 'overdue' && currentBorrow.penalty_amount" class="d-flex align-center">
                                        ...
                                        <v-col cols="12" md="6" class="d-flex align-center">
                                            <v-checkbox v-if="currentBorrow?.is_penalized && currentBorrow?.status === 'overdue'" v-model="finePaid" label="Fine Paid" :disabled="!canExtend" true-icon="fas fa-check-square" false-icon="fas fa-square" />
                                        </v-col>
                                        <v-col cols="12" class="mt-2">
                                            <v-select v-model="extensionDays" :items="extensionOptions" label="Extension Days" density="compact" variant="solo-filled" hide-details="auto" />
                                        </v-col>
                                    </v-row>
                                </div>
                            </v-card>
                ...
                <v-card elevation="2" class="pa-4">
                    <v-row class="mb-4" align="center">
                        <v-col>
                            ...
                            <div class="text-subtitle-2 text-grey">Search or scan a student related to the borrow.</div>
                        </v-col>
                        <v-col cols="auto">
                        ...
                        <v-col cols="12" class="mt-4">
                            <v-card elevation="0" class="pa-4">
                                ...
                                <div v-if="selectedStudent">
                                    ...
                                    <p class="mb-1" v-if="selectedStudent.studentId"><strong>Student ID:</strong> {{ selectedStudent.studentId }}</p>
                                    <p class="mb-1" v-if="selectedStudent.status"><strong>Status:</strong> {{ selectedStudent.status }}</p>
                                    <p class="mb-1" v-if="selectedStudent.id"><strong>Internal ID:</strong> {{ selectedStudent.id }}</p>
                                </div>
        ...
        <v-dialog v-model="scannerOpen" width="700">
            <v-card>
                ...
                <v-card-text class="pa-0">
                    ...
                    <v-expansion-panels class="pa-4">
                        <v-expansion-panel>
                            ...
                            <v-expansion-panel-text>
                                <pre style="white-space:pre-wrap;">{{ JSON.stringify(lastDetectedCodes, null, 2) }}</pre>
                            </v-expansion-panel-text>
                        </v-expansion-panel>
                    </v-expansion-panels>
                    ...
                    <v-row class="pa-4" align="center">
                        <v-col>
                            <v-text-field v-model="simulateInput" label="Paste QR content to simulate" density="compact" variant="solo-filled" hide-details="auto" clearable />
                        </v-col>
                        <v-col cols="auto">
        ...
        <!-- Actions and Recent Extensions (full width) -->
        <v-row class="mt-4">
            <v-col cols="12" class="d-flex justify-end mb-4">
                <v-btn color="primary" :disabled="!canExtend" @click="confirmExtend" size="large" :loading="isExtending">
                    <v-icon left icon="fa-clock-rotate-left"></v-icon>
                    Confirm Extend
                </v-btn>
            </v-col>
            ...
            <v-col cols="12">
                <v-card elevation="2" class="pa-4">
                    <h5 class="mb-2">Recent Extensions</h5>
                    <v-list dense>
                        <v-list-item v-for="(b, idx) in recentExtensions" :key="idx">
                            <v-list-item-content>
...
import { extendBorrowing, getBorrowRecords, updateBorrowRecord, getReturnDetails } from '../services/borrow.js'
...
const extensionDays = ref(null)
const extensionOptions = [3,7,14,30]
const recentExtensions = ref([])
...
function onCameraOn() { cameraStatus.value = 'on' }
function onCameraOff() { cameraStatus.value = 'off' }
...
function onInit(promise) {
    if (promise && typeof promise.then === 'function') {
        promise.then(() => { cameraStatus.value = 'initialized' }).catch(() => { cameraStatus.value = 'error' })
    }
}
...
async function searchByReference() {
    ...
    try {
        ...
        console.log('searchByReference -> details:', details, 'refVal:', refVal)
        ...
        if (borrowRec) {
            const mapped = {
                ...
                is_penalized: !!borrowRec.is_penalized,
            ...
            extensionDays.value = null
            referenceInput.value = ''
        } else {
            const msg = details?.message || 'No active borrow record found for this reference.'
            showDialog('Extension Info', msg, false)
        }
    } catch (err) {
        console.warn('searchByReference failed', err)
        const serverMsg = err?.response?.data?.message || err?.message || 'Failed to fetch borrow details.'
        showDialog('Extension Error', serverMsg, true)
    }
}
    ...
    if (mode === 'book') {
        ...
        if (!fb) {
            ...
            try {
                ...
                console.log('onDecode(book searchKey) -> details:', details, 'searchKey:', searchKey, 'parsed:', parsed)
                const borrowRec = details?.borrow || details?.data || (details && (details.id || details.student || details.book) ? details : null)
                if (borrowRec) {
                    const mapped = {
                        id: borrowRec.id,
        ...
        if (fb) {
            ...
            try {
                ...
                console.log('onDecode(fb.bookCode) -> details:', details, 'fb:', fb)
                const borrowRec = details?.borrow || details?.data || (details && (details.id || details.student || details.book) ? details : null)
                if (borrowRec) {
                    const mapped = {
                        id: borrowRec.id,
...
const canExtend = computed(() => !!selectedBook.value && !!selectedStudent.value)
...
async function confirmExtend() {
    ...
    try {
        if (!currentBorrowId.value && !selectedBook.value?.bookCode) {
            showDialog('Extension Error', 'No matching borrow record found for this book and borrower.', true)
            return
        }

        const payload = {
            reference_number: selectedBook.value?.bookCode || '',
            extension_days: extensionDays.value || 0,
            is_fine_paid: !!finePaid.value
        }

        console.log('Extending borrow with payload', payload)
        const result = await extendBorrowing(payload)
        ...
        recentExtensions.value.unshift(record)
        ...
        console.log('Extend successful:', result)
    } catch (error) {
        console.error('Extend failed:', error)
        const serverMsg = error?.response?.data?.message || error?.message || 'Failed to extend borrowing'
        showDialog('Extension Error', serverMsg, true)
    } finally {
...
async function loadBorrowData() {
    try {
        ...
        console.log('loadBorrowData raw response:', data)
        ...
        console.log('BorrowExtensions loaded borrow records:', borrowRecords.value.length)
    } catch (error) {
...
<style scoped>
.scanner-placeholder { min-height: 200px; display: flex; flex-direction: column; justify-content: center; align-items: center; }
.qrcode-area { height: 480px; display: flex; align-items: center; justify-content: center; background: #f5f7fa; }
</style>
